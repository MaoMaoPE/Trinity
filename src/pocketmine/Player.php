...
class Player extends Human implements CommandSender, InventoryHolder, ChunkLoader, IPlayer {
	...
	public function getLeaveMessage(){
		return new TranslationContainer(ColorFormat::YELLOW . "%multiplayer.player.left", [
			$this->getDisplayName()
		]);
	}
	...
	public function setBanned($value){
		if($value === true){
			...
			$this->kick(ColorFormat::RED . "You have been banned");
		}else{
	...
	protected function doFirstSpawn(){
		...
		$this->server->getPluginManager()->callEvent($ev = new PlayerJoinEvent($this, new TranslationContainer(ColorFormat::YELLOW . "%multiplayer.player.joined", [
			$this->getDisplayName()
		])));
	...
	protected function processLogin(){
		if(!$this->server->isWhitelisted(strtolower($this->getName()))){
			...
			$this->close($this->getLeaveMessage(), ColorFormat::RED . "You are banned");
		...
		$this->server->getLogger()->info($this->getServer()->getLanguage()->translateString("pocketmine.player.logIn", [
			ColorFormat::AQUA . $this->username . ColorFormat::WHITE,
			$this->ip,
			$this->port,
			ColorFormat::GREEN . $this->randomClientId . ColorFormat::WHITE,
			$this->id,
			$this->level->getName(),
			round($this->x, 4),
			round($this->y, 4),
	...
	public function handleDataPacket(DataPacket $packet){
		...
		switch($packet::NETWORK_ID){
			...
			case ProtocolInfo::LOGIN_PACKET:
				...
				$this->username = ColorFormat::clean($packet->username);
			...
			case ProtocolInfo::TEXT_PACKET:
				...
				if($packet->type === TextPacket::TYPE_CHAT){
					$packet->message = ColorFormat::clean($packet->message, $this->removeFormat);
	...
	public final function close($message = "", $reason = "generic reason", $notify = true){
		if($this->connected and !$this->closed){
			...
			$this->server->getLogger()->info($this->getServer()->getLanguage()->translateString("pocketmine.player.logOut", [
				ColorFormat::AQUA . $this->getName() . ColorFormat::WHITE,
				$this->ip,
				$this->port,
